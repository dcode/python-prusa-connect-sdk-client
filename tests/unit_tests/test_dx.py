...
from prusa.connect.client.exceptions import PrusaAuthError
...
def test_client_init_no_creds_raises():
    ...
    with patch("prusa.connect.client.auth.PrusaConnectCredentials.load_default", return_value=None):
...
def test_client_init_auto_load():
    ...
    with patch("prusa.connect.client.auth.PrusaConnectCredentials.load_default", return_value=mock_creds):
